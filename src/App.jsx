...
Responsável: ${data.responsavel || "—"}`;
  ...
  const handleCopiar = () => {
    navigator.clipboard.writeText(preview).then(() => showToast("✓ Mensagem copiada!"));
  };

  const handleWhatsApp = () => {
    const msg = encodeURIComponent(preview);
    window.open(`https://wa.me/${WHATSAPP_GROUP}?text=${msg}`, "_blank");
  };
  ...
  const handleLimpar = () => {
    setForm({ ...empty, responsavel: userName || "" });
    setPreview("");
    setView("form");
  ...
  const handleMelhorar = async () => {
    if (!form.ocorrencia && !form.encaminhamento && !form.situacaoFinal) {
      showToast("Preencha pelo menos um campo de texto.");
      return;
    }
    setImproving(true);
    try {
...
Situação final: ${form.situacaoFinal}`;
  ...
  const FormView = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, paddingBottom: 24 }}>
      ...
      <div style={{
        ...
        <div style={{ color: C.white, fontWeight: 900, fontSize: 16, letterSpacing: 0.3 }}>
          REGISTRO DE OCORRÊNCIA
        </div>
        <div style={{ color: C.muted, fontSize: 11, marginTop: 3 }}>
          Preencha os campos e toque em Finalizar
        </div>
      </div>

      {/* Seção: Dados da ocorrência */}
      <SectionLabel>📅 Dados da ocorrência</SectionLabel>
      <Field label="Data" value={form.data} onChange={f("data")} placeholder="Ex: 19/06/2026" />
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <Field label="Horário início" value={form.horaInicio} onChange={f("horaInicio")} placeholder="Ex: 14:30" />
        <Field label="Horário término" value={form.horaFim} onChange={f("horaFim")} placeholder="Ex: 15:10" />
      </div>
      <Field label="Local" value={form.local} onChange={f("local")} placeholder="Ex: Plataforma 4/5 — Estação Brás" />

      {/* Seção: Passageiro */}
      <SectionLabel>👤 Dados do passageiro</SectionLabel>
      <Field label="Passageiro(a)" value={form.passageiro} onChange={f("passageiro")} placeholder="Nome completo" />
      <Field label="CPF" value={form.cpf} onChange={f("cpf")} placeholder="000.000.000-00" />
      <Field label="Endereço" value={form.endereco} onChange={f("endereco")} placeholder="Rua, número, bairro, cidade" />
      <Field label="Telefone" value={form.telefone} onChange={f("telefone")} placeholder="(11) 9 0000-0000" />

      {/* Seção: Relato */}
      <SectionLabel>📋 Relato</SectionLabel>
      <Field label="Ocorrência" value={form.ocorrencia} onChange={f("ocorrencia")}
        placeholder="Descreva o que aconteceu..." multiline rows={5} />
      <Field label="Encaminhamento" value={form.encaminhamento} onChange={f("encaminhamento")}
        placeholder="Medidas tomadas, acionamentos..." multiline rows={4} />
      <Field label="Situação final" value={form.situacaoFinal} onChange={f("situacaoFinal")}
        placeholder="Como a situação foi encerrada..." multiline rows={3} />

      {/* Seção: Encerramento */}
      <SectionLabel>✍️ Encerramento</SectionLabel>
      <Field label="Testemunha" value={form.testemunha} onChange={f("testemunha")} placeholder="Nome da testemunha (se houver)" />
      <Field label="Responsável" value={form.responsavel} onChange={f("responsavel")} placeholder="Seu nome" />
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <button onClick={handleSalvar} style={btnStyle("rgba(34,197,94,0.12)", "rgba(34,197,94,0.4)", C.green)}>
          💾 Salvar
        </button>
        <button onClick={handleLimpar} style={btnStyle("rgba(239,68,68,0.1)", "rgba(239,68,68,0.3)", C.red)}>
          🗑 Limpar
        </button>
      </div>
      ...
      <button
        ...
        style={{
          ...
          color: "white", fontSize: 16, fontWeight: 900, cursor: "pointer", letterSpacing: 0.3,
  ...
  const PreviewView = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, paddingBottom: 24 }}>
      ...
      {/* Ações */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <button onClick={handleCopiar} style={btnStyle("rgba(245,158,11,0.12)", "rgba(245,158,11,0.4)", C.amber)}>
          📋 Copiar
        </button>
        <button onClick={handleSalvar} style={btnStyle("rgba(34,197,94,0.12)", "rgba(34,197,94,0.4)", C.green)}>
          💾 Salvar
      ...
      <button
        ...
        style={{
          background: "#128C7E", border: "none", borderRadius: 9, padding: "15px",
      ...
      {saved.length === 0 ? (
        ...
        <div key={oc.id || i} style={{
          ...
          <div style={{ color: C.muted, fontSize: 12, marginBottom: 10 }}>
            {oc.passageiro || "Passageiro não informado"} · {oc.responsavel}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button
              onClick={() => {
                navigator.clipboard.writeText(oc.texto || buildText(oc));
                showToast("✓ Copiado!");
              }}
              style={{
                background: "transparent", border: `1px solid ${C.border}`,
                borderRadius: 6, padding: "6px 12px",
                color: C.muted, fontSize: 11, fontWeight: 600, cursor: "pointer", flex: 1,
              }}>
              📋 Copiar
            </button>
            <button
              onClick={() => {
                const msg = encodeURIComponent(oc.texto || buildText(oc));
                window.open(`https://wa.me/${WHATSAPP_GROUP}?text=${msg}`, "_blank");
              }}
              style={{
                background: "rgba(18,140,126,0.15)", border: "1px solid rgba(18,140,126,0.4)",
                borderRadius: 6, padding: "6px 12px",
                color: "#25D366", fontSize: 11, fontWeight: 700, cursor: "pointer", flex: 1,
              }}>
              WhatsApp
            </button>
          </div>
