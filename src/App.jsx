...
function RegisterScreen({ onRegister, loading }) {
  ...
  const addLog = (name, type) => {
    const ts = new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
    setLog(prev => [{ name, type, ts, id: Date.now() }, ...prev].slice(0, 30));
  };
  ...
  useEffect(() => {
    ...
    load("https://download.agora.io/sdk/release/AgoraRTC_N.js")
      .then(() => setAgoraReady(true))
      .catch(() => setError("Falha ao carregar SDK de áudio."));
  }, []);

  // Entra no canal ao montar
  useEffect(() => {
    if (!agoraReady || AGORA_APP_ID === "SEU_AGORA_APP_ID") return;
    let mounted = true;
    ...
    const join = async () => {
      try {
        ...
        AgoraRTC.setLogLevel(4); // silencia logs

        const client = AgoraRTC.createClient({ mode: "rtc", codec: "vp8" });
        clientRef.current = client;

        // Escuta quem está publicando (alguém apertou PTT)
        client.on("user-published", async (remoteUser, mediaType) => {
          if (mediaType === "audio") {
            await client.subscribe(remoteUser, "audio");
            remoteUser.audioTrack.play();
            if (mounted) {
              setSpeaking(remoteUser.uid);
              setStatus("listening");
            }
          }
        });
        ...
        await client.join(AGORA_APP_ID, AGORA_CHANNEL, null, userName);
        if (mounted) setStatus("ready");
      } catch (e) {
        if (mounted) { setError("Erro ao entrar no canal: " + e.message); setStatus("idle"); }
      }
    };
    join();
    ...
    return () => {
      mounted = false;
      if (localTrack.current) { localTrack.current.stop(); localTrack.current.close(); }
      clientRef.current?.leave();
    };
  }, [agoraReady, userName]);

  // Sinaliza via Firebase quem está falando (para mostrar o nome nos outros)
  const signalTalking = (talking) => {
    if (!_db) return;
    _db.ref("guarnicao/radio").set(talking ? { user: userName, ts: Date.now() } : null);
  };

  // Escuta sinalização Firebase para mostrar nome de quem fala
  useEffect(() => {
    if (!_db) return;
    const ref = _db.ref("guarnicao/radio");
    ref.on("value", snap => {
      const data = snap.val();
  ...
  const startTalking = async () => {
    ...
    try {
      ...
      const track = await AgoraRTC.createMicrophoneAudioTrack({
        encoderConfig: { sampleRate: 16000, stereo: false, bitrate: 24 },
  ...
  const stopTalking = async () => {
    ...
    try {
      if (localTrack.current) {
        await clientRef.current.unpublish([localTrack.current]);
  ...
  const notConfigured = AGORA_APP_ID === "SEU_AGORA_APP_ID";

  const pttColor   = status === "talking"   ? "#22C55E"
                   : status === "listening" ? "#F59E0B"
                   : status === "ready"     ? C.blue
                   : C.muted;

  const pttBg      = status === "talking"   ? "rgba(34,197,94,0.15)"
                   : status === "listening" ? "rgba(245,158,11,0.1)"
                   : status === "ready"     ? "rgba(26,111,212,0.12)"
                   : C.card;

  const pttLabel   = status === "talking"   ? "🎙 TRANSMITINDO..."
                   : status === "listening" ? `📻 ${speaking || "Alguém"} está falando...`
                   : status === "connecting" ? "Conectando ao canal..."
                   : status === "ready"     ? "Segure para falar"
                   : "Aguardando...";
  ...
  return (
    <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: 14, paddingBottom: 40 }}>
      ...
      <div style={{
        ...
        borderLeft: `3px solid ${C.green}`, borderRadius: 10, padding: "14px 16px",
        display: "flex", alignItems: "center", justifyContent: "space-between",
      }}>
        <div>
          <div style={{ color: C.white, fontWeight: 800, fontSize: 15 }}>📻 Canal Guarnição</div>
          <div style={{ color: C.muted, fontSize: 11, marginTop: 2 }}>
            Push-to-talk · Voz em tempo real
          </div>
        </div>
        <div style={{
          display: "flex", alignItems: "center", gap: 6,
          padding: "5px 12px", borderRadius: 6,
          background: status === "ready" || status === "talking" || status === "listening"
            ? "rgba(34,197,94,0.1)" : "rgba(74,85,104,0.2)",
          border: `1px solid ${status === "ready" || status === "talking" || status === "listening"
            ? "rgba(34,197,94,0.3)" : C.border}`,
        }}>
          <span style={{
            width: 7, height: 7, borderRadius: "50%", display: "inline-block",
            background: status === "ready" || status === "talking" || status === "listening"
              ? C.green : C.muted,
            animation: status === "talking" || status === "listening" ? "pulse 1s infinite" : "none",
          }}/>
          <span style={{
            fontSize: 11, fontWeight: 700,
            color: status === "ready" || status === "talking" || status === "listening"
              ? C.green : C.muted,
          }}>
            {status === "connecting" ? "Conectando" : status === "idle" ? "Offline" : "Online"}
          </span>
        </div>
      </div>
      ...
      {notConfigured && (
        <div style={{
          ...
          <div style={{ color: C.amber, fontWeight: 800, fontSize: 14 }}>⚙ Configuração necessária</div>
          <div style={{ color: C.muted, fontSize: 12, lineHeight: 1.6 }}>
            Para ativar o rádio, crie uma conta gratuita em{" "}
            <a href="https://console.agora.io" target="_blank" rel="noreferrer"
              style={{ color: C.blue }}>console.agora.io</a>,
            crie um projeto, copie o <strong style={{ color: C.text }}>App ID</strong> e
            substitua <code style={{ color: C.amber }}>SEU_AGORA_APP_ID</code> no topo do arquivo.
          </div>
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16, padding: "20px 0" }}>
          ...
          {(status === "talking" || status === "listening") && (
            <>
              <div style={{
                ...
                border: `2px solid ${pttColor}`, opacity: 0.4,
                animation: "ringPulse 1.2s ease-out infinite",
              }}/>
              <div style={{
                position: "absolute", width: 140, height: 140, borderRadius: "50%",
                border: `2px solid ${pttColor}`, opacity: 0.25,
                animation: "ringPulse 1.2s ease-out infinite 0.3s",
              }}/>
            </>
          )}
          <button
            onPointerDown={!notConfigured ? startTalking : undefined}
            onPointerUp={!notConfigured ? stopTalking : undefined}
            onPointerLeave={!notConfigured ? stopTalking : undefined}
            disabled={notConfigured || status === "connecting" || status === "listening"}
            style={{
              width: 120, height: 120, borderRadius: "50%",
              background: pttBg,
              border: `3px solid ${pttColor}`,
              cursor: notConfigured || status === "listening" ? "not-allowed" : "pointer",
              display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
              gap: 6, transition: "all 0.15s",
              boxShadow: status === "talking" ? `0 0 32px ${pttColor}55` : "none",
              outline: "none",
            ...
            <span style={{ color: pttColor, fontSize: 9, fontWeight: 800, textTransform: "uppercase", letterSpacing: 0.8 }}>
        ...
        {/* Status label */}
        <div style={{
          color: pttColor, fontSize: 14, fontWeight: 700, textAlign: "center",
          minHeight: 22,
          animation: status === "talking" ? "blink 1s infinite" : "none",
        }}>
          {pttLabel}
        </div>
